...
## Problem 1: Analysis of Network File
a). a summary of important statistics of the network — number of edges, number of two and three stars, number of triangles.
```{r message=FALSE}
...
b). Based on the above information what can you say about the network? Comment on how the different statistics relate to each other. What model do you think could have generated this graph?  
• Since "2-stars"" number is only around 3 times of "edges"", the graph is not very centralized, otherwise, the "2-star"" numbers will be much higher. The "triangle" number is only 5, which means the edge distribution is very random evenly across the graph. This graph might generated from **Erdos-Renyi np model**.   

c). Fit the simplest model: and check if it adequately fits the other summary statistics.  
```{r message=FALSE}
# c) model with ergm
...
d). How does the fit look? For example, report the fraction of simulated networks that have values of their statistic that are greater than the observed one. Which of the statistics is fit least well?  
• From the plots in c), this fitting does not work very well. Significant amount of "2-star/3-star or triangles" with higher simulated result compared to the observed data of the original graph. 3-Star is especially fitted badly as we can see from the histogram.  

e). Fit a new model where you add the statistic that you chose from above to the model in 1.c. Repeat the model fit test.
```{r message=FALSE}
...
f). How does the fit look now? Do any other statistics look like they fit better now? Have AIC and BIC improved? 
```{r message=FALSE}
...
• As shown in the histograpms, this fitting generate much better fitting with other statistics, i.e. "3-star" and "triangles".  


g). Consider the other models that include some or all of the statistics 2-star, 3-star, triangles. Fit them all and report AIC and BIC. What is the best model according to these criteria?
```{r message=FALSE}
...
This simulated statistics did not converge. The statistics heads off into groups with much higher density. This is indication of degeneracy. 
...
# With depth = 1 for karate data
gpList

depth <- 2
gpList <- concordWithAdjmatrix(adj.matrix, depth)
# With depth = 2 for karate data
gpList

```

Algorithm Implementation:
I implemented the CONCORD algorithm with self-implemented functions, the input needs "adjacent matrix" of the network and user defined depth. 
I tried with depth equals to 1 or 2, the output is shown in the graph. 
we can keep divide the groups, but, from depth = 1, we get consistent data as the karate paper. 
